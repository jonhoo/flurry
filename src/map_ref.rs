...
/// A reference to a [`HashMap`], constructed with [`HashMap::pin`].
///
...
pub struct HashMapRef<'map, SH, K, V, S = crate::DefaultHashBuilder> {
...
impl<K, V, S> HashMap<K, V, S> {
    ...
    pub fn pin<'m>(&'m self) -> HashMapRef<'m, impl Shield<'m>, K, V, S> {
        HashMapRef {
            guard: self.guard(),
            map: &self,
        }
...
impl<'m, SH, K, V, S> HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
{
    ...
    pub fn iter<'g>(&'g self) -> Iter<'m, 'g, K, V, SH> {
    ...
    pub fn keys<'g>(&'g self) -> Keys<'m, 'g, K, V, SH> {
    ...
    pub fn values<'g>(&'g self) -> Values<'m, 'g, K, V, SH> {
...
impl<'m, 'g, SH, K, V, S> IntoIterator for &'g HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
{
    type IntoIter = Iter<'m, 'g, K, V, SH>;
    type Item = (&'g K, &'g V);
...
impl<'m, SH, K, V, S> Debug for HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
    K: Debug,
    V: Debug,
...
where
    ...
    S: BuildHasher,
{
    fn eq(&self, other: &HashMapRef<'m2, SH2, K, V, S>) -> bool {
        self.map.guarded_eq(&other.map, &self.guard, &other.guard)
    }
}

impl<'m, SH, K, V, S> PartialEq<HashMap<K, V, S>> for HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
    K: Hash + Ord,
    V: PartialEq,
...
impl<'m, SH, K, V, S> PartialEq<HashMapRef<'m, SH, K, V, S>> for HashMap<K, V, S>
where
    SH: Shield<'m>,
    K: Hash + Ord,
    V: PartialEq,
    S: BuildHasher,
{
    fn eq(&self, other: &HashMapRef<'m, SH, K, V, S>) -> bool {
        self.guarded_eq(&other.map, &self.guard(), &other.guard)
    }
}

impl<'m, SH, K, V, S> Eq for HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
    K: Hash + Ord,
    V: Eq,
...
impl<'m, SH, K, Q, V, S> Index<&'_ Q> for HashMapRef<'m, SH, K, V, S>
where
    SH: Shield<'m>,
    K: Hash + Ord + Borrow<Q>,
    Q: ?Sized + Hash + Ord,

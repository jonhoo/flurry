...
/// The current thread will be pinned for the duration of this reference.
/// Keep in mind that this prevents the collection of garbage generated by the map.
pub struct HashMapRef<'map, K: 'static, V: 'static, L, S = crate::DefaultHashBuilder>
where
    L: lock_api::RawMutex,
    S: BuildHasher,
{
    ...
    L: lock_api::RawMutex,
{
    ...
    pub fn pin(&self) -> HashMapRef<'_, K, V, L, S> {
        HashMapRef {
    ...
    pub fn with_guard<'g>(&'g self, guard: &'g Guard) -> HashMapRef<'g, K, V, L, S> {
        HashMapRef {
            map: &self,
...
impl<K, V, L, S> HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send,
    S: BuildHasher,
...
impl<'g, K, V, L, S> IntoIterator for &'g HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send,
    S: BuildHasher,
...
impl<K, V, L, S> Debug for HashMapRef<'_, K, V, L, S>
where
...
#[cfg(feature = "std")]
impl<K, V, L, S> Clone for HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send,
    S: BuildHasher,
...
impl<K, V, L, S> PartialEq for HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
...
impl<K, V, L, S> PartialEq<HashMap<K, V, L, S>> for HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send + PartialEq,
    S: BuildHasher,
    L: lock_api::RawMutex,
{
    fn eq(&self, other: &HashMap<K, V, L, S>) -> bool {
        self.map.guarded_eq(&other, &self.guard, &other.guard())
    }
}

impl<K, V, L, S> PartialEq<HashMapRef<'_, K, V, L, S>> for HashMap<K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send + PartialEq,
    S: BuildHasher,
    L: lock_api::RawMutex,
{
    fn eq(&self, other: &HashMapRef<'_, K, V, L, S>) -> bool {
        self.guarded_eq(&other.map, &self.guard(), &other.guard)
    }
}

impl<K, V, L, S> Eq for HashMapRef<'_, K, V, L, S>
where
    K: Sync + Send + Clone + Hash + Eq,
    V: Sync + Send + Eq,
    S: BuildHasher,
...
impl<K, Q, V, L, S> Index<&'_ Q> for HashMapRef<'_, K, V, L, S>
where
